    ...
    internal class PageParser : IPageParser
    {
        public SearchResults ParseSearchResults(SearchRequest request, Stream content)
        {
            ...
            var rows = doc.DocumentNode
                .SelectSingleNode("//ol[@class='cl-static-search-results']")
                .ChildNodes
                .TakeWhile(n => !n.HasClass("nearby"))
                .Where(n => n.HasClass("cl-static-search-result"));
            ...
            {
                NextPageUrl = next,
                Results = rows.Select(ParseRow).ToArray()
            };
        }
        ...
        private static SearchResult ParseRow(HtmlNode row)
        {
            ...
            if (url != null)
            {
                id = Regex.Match(url, @"\d+[.]html").Value.Replace(".html", "");
            }

            var title = row.Attributes["title"].Value;
